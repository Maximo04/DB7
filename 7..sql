...
SELECT c.first_name, c.last_name, a.address, (SELECT MIN(amount) FROM payment p WHERE c.customer_id =p.customer_id) AS lowest_payment FROM customer c JOIN address a ON c.address_id =a.address_id; 
--Generate a report that shows the customer's information with the highest payment and the lowest payment in the same row.
SELECT c.first_name, c.last_name, a.address, (SELECT MIN(amount) FROM payment p WHERE c.customer_id =p.customer_id) AS lowest_payment,(SELECT MAX(amount) FROM payment p WHERE c.customer_id =p.customer_id) AS highest_payment  FROM customer c JOIN address a ON c.address_id =a.address_id; 